...
export default function Timeline({ dates, selectedIndex, onSelect }: TimelineProps) {
	...
	const computeLabels = useCallback(
		throttle(() => {
			...
			const labels = indices.map((dataIndex, position) => {
				...
				let left = fraction * sliderWidth - approxLabelWidth / 2;
	...
	return (
		<div className="pointer-events-auto select-none text-neutral-100 bg-neutral-800/40 rounded px-12 pt-1 pb-1" aria-label="Timeline">
			<div className="relative h-10">
				...
				<input
					...
					className="absolute inset-0 w-full h-6 appearance-none cursor-pointer timeline-slider"
					role="slider"
					tabIndex={0}
					aria-valuemin={0}
				...
				<div className="absolute top-6 left-0 right-0">
					{labelData.map(({ index, left, text, leftPercent, alignment }) => (
						<div
							key={index}
							className="absolute text-[10px] text-neutral-300 whitespace-nowrap text-center"
							style={{
